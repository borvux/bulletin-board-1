class BoardsController < ApplicationController
  ...
  def show
    ...
    @active_posts = Post.where({ :board_id => @the_board.id }).order({ :expires_on => :asc })

    @expired_posts = Post.where({ :board_id => @the_board.id }).where("posts.expires_on >= ?", Date.current).order({ :expires_on => :desc })
